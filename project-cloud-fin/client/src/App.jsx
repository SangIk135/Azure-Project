...
const mockPopularArtists = [
    ...
    { id: 'artist-8', name: 'IU', imageUrl: 'https://i.ibb.co/Q8Q8cM9/iu.jpg' },
];
            ...
            <FormContainer>
                ...
                <Form onSubmit={handleSubmit}>
                    <FormGroup>
                        ...
                        <Input
                            auth type="text" id="email" placeholder="아이디 또는 이메일" required
            ...
            <div style={{display: 'flex', gap: '2rem', marginBottom: '2rem', alignItems: 'flex-end'}}>
                ...
                <div>
                    ...
                    <p style={{color: '#94a3b8', marginTop: '1rem'}}>
                        <span>by {playlist.creatorNickname}</span><span style={{margin: '0 0.5rem'}}>•</span><span>{playlist.songs.length}곡</span>
                    </p>
                </div>
            </div>
            {isOwner && (
...
function CreatePlaylistPage({ setPage, addPlaylist, user }) {
    const [error, setError] = useState('');
    ...
    return (
        <PageContainer>
            ...
            <Form onSubmit={handleCreate} style={{maxWidth: '600px', gap: '1.5rem'}}>
                <FormGroup style={{gap: '0.5rem'}}> <label htmlFor="name">이름</label> <Input type="text" id="name" name="name" required placeholder="플레이리스트 제목" /> </FormGroup>
                <FormGroup style={{gap: '0.5rem'}}><label htmlFor="description">설명</label> <Input as="textarea" rows="4" id="description" name="description" placeholder="플레이리스트에 대한 간단한 설명" /> </FormGroup>
                <FormGroup style={{flexDirection: 'row', alignItems: 'center', gap: '1rem'}}>
                     <input type="checkbox" id="is_public" name="is_public" style={{width: '1rem', height: '1rem'}}/>
                    ...
                    results.map((song) => (
                        <SongItem key={song.spotifyUrl}>
                            ...
                            <a href={song.spotifyUrl} target="_blank" rel="noopener noreferrer">
                              <Button style={{ fontSize: '0.875rem', padding: '0.4rem 0.8rem', marginTop: 0 }}>Spotify</Button>
                            </a>
                        </SongItem>
                    ))
                ) : (
...
export default function App() {
  const [page, setPage] = usePageWithHistory('login');
  const [user, setUser] = useState(null);
  const [context, setContext] = useState({});
  const [playlists, setPlaylists] = useState([]);

  // 기존 addPlaylist/deletePlaylist는 서버 API로 대체
  const addPlaylist = () => {};
  ...
  useEffect(() => {
      const protectedPages = ['myPlaylists', 'createPlaylist', 'playlistDetail', 'publicPlaylists'];
  ...
  const renderPage = () => {
    switch (page) {
      ...
      case 'createPlaylist': return <CreatePlaylistPage setPage={setPage} addPlaylist={addPlaylist} user={user} />;
      case 'search': return <SearchPage context={context} />;
      default: return <LoginPage setPage={setPage} setUser={setUser} />;
    }
  };
